 ...
 app.factory('httpService', ['$http', '$mdToast', httpService]);
 ...
 function httpService($http, $mdToast){
 	var serviceFactory = {};
 	var last = {
 		bottom: false,
 		top: true,
    	...
    	vm.fileForm = {text: '', file: {}, submit: function(fileForm){
    		...
    		if(!vm.fileForm.file.required && fileForm.$valid){
    			...
    			call.then(function(response){
    				httpService.toast(response.data.success);
    			},function(response){
    				httpService.toast(response.data.error);
    			});
    		}
    	}};
